...
OFFICERS = [s for s in (os.getenv("OFFICERS") or "").replace(" ", "").split(",") if s] or ["@Maffins89","@Gi_Di_Al","@oOMEMCH1KOo","@Ferbi55","@Ahaha_Ohoho"]
...
def is_leader(message: types.Message) -> bool:
    if LEADER_ID:
        if str(LEADER_ID).startswith("@") and (message.from_user.username or ""):
            if ("@" + (message.from_user.username or "").lower()) == str(LEADER_ID).lower():
                return True
        try:
